...
/**
 * GitHub の Pull Request テンプレート（PR作成時に説明欄へ自動展開される）。
 * レビュー観点を毎回ゼロから書かずに済むよう、Salesforce開発で確認すべき項目
 * （テスト/カバレッジ・デプロイ先・破壊的変更）をチェックリスト化する。GitHub Flow の定番。
 */
export function pullRequestTemplate(): string {
  return `<!-- Generated by Salesforce Dev Manager — チームで使うPRの雛形です。不要な行は消してOK。 -->
...
## 確認したこと
- [ ] ローカル/開発環境にデプロイして動作確認した
- [ ] Apexテストが通った（必要なカバレッジを満たす）
- [ ] 影響範囲（他機能・権限・レイアウト）を確認した

## デプロイに関する注意
- [ ] 破壊的変更（項目/オブジェクト削除など）は **無い**
<!-- ある場合は、影響と手順をここに記載 -->

## 関連
<!-- Issue番号・チケットURLなど（例: #123） -->
`;
}
...
export function cicdFiles(config: TeamflowConfig): ScaffoldFile[] {
  return [
    ...
    { relativePath: ".github/pull_request_template.md", content: pullRequestTemplate() },
  ];
}
