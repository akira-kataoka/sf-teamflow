...
- 表示: ホームの「📝 保存される変更 N件」と、**保存時の種別選択ピッカーのタイトル**の両方に**種別内訳**（例「新規2・変更3・削除1」）を併記し、保存範囲を一目で把握できる（`summarizeChangeCounts`・**全件から算出**＝一覧が先頭40件のみでも内訳は正確・分かりやすい順で並べ単体テスト済）。ファイル一覧自体も**種別順（新規→変更→削除→…）＋同種内はパス昇順**に整列し（`sortByChangeType`）、内訳ラベルと並びが一致して走査しやすい。**各ファイル行はクリック/Enterで開ける**（保存前に「何を変えたか」を確認）。削除済みファイルは開けないため非クリック（`openFile` 経由・既存の競合一覧と同じ仕組み）
...
- 取り込み済みブランチの掃除（✅ 実gitで統合テスト）: ブランチ管理の先頭に「🧹 取り込み済みブランチを掃除（マージ済み）」を追加（新規タイルは増やさない）。`git branch --merged` から現在ブランチに取り込み済みのものを `parseMergedBranches` で抽出し、**現在ブランチ・保護ブランチ（main/develop/release等）を除外**して複数選択（既定全選択）→確認→`-d` で一括削除。PR取り込み後に溜まる古い feature ブランチの片付けを、変更を失わずに安全に行える（未マージは候補に出ない／`-d` なので保険も二重）
...
- ピッカー: 履歴/取り消しの両方で説明行を共通化（`formatCommitMeta`：👤著者・相対日時・ハッシュ・マージ注記）し、**`matchOnDescription` で著者名やハッシュからも絞り込める**（長い一覧から特定コミットを探しやすい・単体テスト済）
...
- **無効タイルの理由表示**: 前提未達のタイルは、ツールチップに「なぜ押せないか＝次に何をすれば押せるか」を表示（project→プロジェクト作成 / org→環境を認証 / repo→バックアップでGit開始 / remote→GitHubに接続）。`disabled` 属性だとホバーで title が出ないため **`aria-disabled`+クラス**にし（クリックは `activateFrom` で無視・`tabindex=-1` でタブ順から除外）、グレーアウトの見た目は維持しつつ理由を伝える
