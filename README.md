...
- 🔄 **GitHubと同期**（pull→push）
- 🌿 **ブランチ管理**（切替/**取り込み(マージ)**/作成（**種別 feature・hotfix・release を選んで命名**）/削除。競合はホームの一覧で解決、または **🛑 中止して取り込み前に戻す**）/ 🔀 **Pull Request作成** / 🏷️ **タグ管理**（次バージョンを semver で選択：修正/新機能/破壊的。push後に **GitHubリリース**もノート自動生成で作成可）
...
| 保存 | 作業ブランチを作成 / ブランチを管理 | 種別(feature/hotfix/release)を選んで命名・切替/**取り込み(マージ)**/作成/削除/**取り込み中止(競合からの脱出)**/**🧹取り込み済みブランチを一括掃除** |
